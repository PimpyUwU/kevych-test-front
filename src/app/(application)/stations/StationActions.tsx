...
export default function StationActions({
    ...
    return (
        <div className="flex space-x-2">
            <Link href={`/stations/edit/${stationId}`} className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-md hover:bg-indigo-200 transition-colors duration-200">
                Edit
            </Link>
            <Link href={`routes?station_id=${stationId}`} className="px-3 py-1 bg-purple-100 text-purple-700 rounded-md hover:bg-purple-200 transition-colors duration-200">
                Routes
            </Link>
            <button
